...
functionname='heart_ASM_2D_example.m'; functiondir=which(functionname);
...
for i=1:5
   PrepareHeartData(['HeartFotos/0' int2str(i) '.bmp'],... 
   ['HeartFotos/0' int2str(i) 'k.bmp'],...
   ['0' int2str(i)])
end
...
for i=1:5
  %  filename=['..\images2D\contour' number '.mat'];
    %load(filename);
    filename=['HeartFotos/0' int2str(i) '.bmp'];
    I=im2double(imread(filename));  
    filename=['HeartFotos/0' int2str(i) '.mat'];
...
%% Test the ASM model %%
Itest=im2double(imread('HeartFotos/06.bmp'));
